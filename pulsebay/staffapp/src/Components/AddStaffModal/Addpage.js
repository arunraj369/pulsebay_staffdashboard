...
const AddStaffModal = ({ show, onClose, onSave }) => {
  ...
  const handleSave = () => {
    ...
    if (Object.keys(validationErrors).length === 0) {
      // If no validation errors, save the staff
      onSave({ name, email, address, dob });
      setName("");
      setEmail("");
      setAddress("");
      setDob("");
  ...
  const handleClose = () => {
    // Clear input fields and errors when modal is closed
    setName("");
    setEmail("");
    setAddress("");
    setDob("");
